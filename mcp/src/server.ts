/**
 * HTTP Server för MCP (lokal/valfri)
 *
 * Denna server exponerar MCP över HTTP. Standard-transporten är stdio
 * (se src/index.ts) — denna HTTP-variant är valfri för lokala test eller
 * självhostning.
 */
...
function createApp() {
  ...
  app.post("/mcp/call-tool", async (req, res) => {
    try {
      ...
      const result = await callMCPHandler("tools/call", {
        name,
        arguments: args || {},
      });
    ...
    try {
      ...
      if (method === "initialize") {
        const initResult = {
          ...
          serverInfo: {
            ...
            version: "2.1.0",
            description:
              "MCP Server för Riksdagen och Regeringskansliet med öppna API-verktyg",
          },
        };
          ...
          prompts: [
            {
              ...
              arguments: [
                {
                  name: "query",
                  description: "Sökord, titel eller fritext",
                  required: false,
                },
                {
                  name: "document_type",
                  description: "Dokumenttyp (prop, bet, mot, etc.)",
                  required: false,
                },
                {
                  name: "rm",
                  description: "Riksmöte (2023/24)",
                  required: false,
                },
              ],
            },
            {
              ...
              arguments: [
                {
                  name: "namn",
                  description: "Förnamn, efternamn eller sökfråga",
                  required: false,
                },
                {
                  name: "parti",
                  description: "Parti (S, M, SD)",
                  required: false,
                },
            ...
            },
            {
              ...
              description:
                "Sök voteringar med beteckning, punkt eller gruppering",
              arguments: [
                { name: "rm", description: "Riksmöte", required: false },
                { name: "bet", description: "Beteckning", required: false },
                {
                  name: "groupBy",
                  description: "Gruppera per parti, valkrets eller namn",
                  required: false,
                },
              ],
            },
            ...
            },
            {
              name: "fetch-report",
              description:
                "Hämtar en rapport (ledamotsstatistik, könsstatistik, diarium)",
              arguments: [
                {
                  name: "report",
                  description:
                    "Identifierare (ledamotsstatistik, kontaktutskott, aldersstatistik, konstatsstatistik, mandatperiod, diarium)",
                  required: true,
                },
              ],
            },
      ...
      if (method === "prompts/get") {
        ...
        const prompts: Record<string, any> = {
          "search-documents": {
            description:
              "Sök dokument med titlar, beteckningar eller ämnesattribut",
            messages: [
              {
                role: "user",
                content: {
...
async function main() {
  try {
    ...
    app.listen(PORT, () => {
      ...
      logger.info(
        `  POST /mcp/list-tools - List available tools (legacy, with auth)`,
      );
      logger.info(`  POST /mcp/call-tool - Call a tool (legacy, with auth)`);
      logger.info(
        `  POST /mcp/list-resources - List available resources (legacy, with auth)`,
      );
      logger.info(
        `  POST /mcp/read-resource - Read a resource (legacy, with auth)`,
      );
    });
  } catch (error) {
